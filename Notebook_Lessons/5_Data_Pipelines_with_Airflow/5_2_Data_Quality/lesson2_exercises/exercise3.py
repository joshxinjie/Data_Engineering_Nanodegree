...
create_trips_table = PostgresOperator(
    ...
    # Alternatively, can type a SQL query instead
...
location_traffic_task = PostgresOperator(
    ...
    dag=dag,
    postgres_conn_id="redshift",
    # If we only want to perform the select statements based on data from 
    # a particular time period
    # Use prev_ds and next_ds to scope the location traffic task
    # Use four brackets because we are formatting twice
    # First, we run the python format which is going to insert the SQL 
    # statement, that will remove two brackets
    # Still need Airflow to inject the previous and next execution date 
    # into the SQL statement when it runs
    sql=f"""
